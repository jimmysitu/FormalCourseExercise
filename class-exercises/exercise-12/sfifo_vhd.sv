...
//
module sfifo_vhd(i_clk, i_reset, i_wr, i_data, o_full, i_rd, o_data, o_empty,
		o_err, wraddr, rdaddr, r_next, r_unfl, r_ovfl, 
		f_const_addr, f_const_next_addr,
		f_data_first, f_data_next);
	...
	input	wire		o_full;		// True if there's no more space
	// Read interface
	input	wire		i_rd;
	input	wire [(DW-1):0]	o_data;
	input	wire		o_empty;	// True if FIFO is empty
	// 
	input	wire		o_err;		// True following under/overflow
	//
